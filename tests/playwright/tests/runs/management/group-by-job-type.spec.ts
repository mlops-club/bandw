...
test('group by job type: table loads with run data', async ({ authedPage, targetConfig, sdkData }) => {
...
test('group by job type: Group button is accessible', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
