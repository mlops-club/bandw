...
test('summary-metric-deltas: workspace loads with charts', async ({ authedPage, targetConfig, sdkData }) => {
...
test('summary-metric-deltas: runs table loads with data', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
...
test('summary-metric-deltas: table has metric columns', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
  ...
  // Verify metric column headers are present
  await expect(page.getByText(/loss|accuracy|val/i).first()).toBeVisible({ timeout: 10_000 });
});
