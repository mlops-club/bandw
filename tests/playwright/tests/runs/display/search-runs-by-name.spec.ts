...
test('search runs: table loads with run data', async ({ authedPage, targetConfig, sdkData }) => {
...
test('search runs: search input is accessible', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
...
test('search runs: multiple runs visible in table', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
