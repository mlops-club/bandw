...
test('color by config key: workspace loads with charts', async ({ authedPage, targetConfig, sdkData }) => {
...
test('color by config key: workspace settings button exists', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
