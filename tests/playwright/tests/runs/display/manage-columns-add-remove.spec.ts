...
test('manage columns: table loads with run data', async ({ authedPage, targetConfig, sdkData }) => {
...
test('manage columns: Columns button is accessible', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
  ...
  // Verify Columns button exists
  await expect(page.getByRole('button', { name: /Columns/i }).first()).toBeVisible({ timeout: 10_000 });
});

test('manage columns: table has column headers', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
  ...
  await expect(page.getByText(/Name|State|Created/i).first()).toBeVisible({ timeout: 10_000 });
});
