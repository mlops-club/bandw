...
test('create-scatter-plot: workspace loads and panel picker accessible', async ({ authedPage, targetConfig, sdkData }) => {
  ...
  // Verify panel picker opens with chart types
  await expect(page.getByText(/Scatter|Bar chart|Parallel/i).first()).toBeVisible({ timeout: 10_000 });
});

test('create-scatter-plot: workspace has chart content', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
...
test('create-scatter-plot: workspace settings controls exist', async ({ authedPage, targetConfig, sdkData }) => {
  const page = authedPage;
