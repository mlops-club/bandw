...
4. Tests receive `{ project, entity, runs }` and navigate to `/{entity}/{project}/...`
...
**Priority order** (aria-first, fallback to test IDs for canvas):

1. `getByRole('table')`, `getByRole('row')`, `getByRole('cell')`
2. `getByRole('tab', { name: /overview/i })`
3. `getByRole('searchbox', { name: /search/i })`
4. `getByRole('button', { name: /filter/i })`
5. `getByRole('link', { name: runDisplayName })`
6. `getByLabel('...')`
7. `getByTestId('chart-{key}')` — **fallback only** for `<canvas>` elements
...
Every test run — against **both** targets — records all network traffic through a `network-recorder.ts` fixture. This is **always-on**, not opt-in. The recordings are not used as assertions; they are a corpus for offline analysis of how our backend/frontend differs from the real W&B.
...
- **Filter option**: can be narrowed to `/graphql` and `/files/` if full HAR is too noisy
