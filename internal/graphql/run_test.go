...
func TestUpsertBucketCreatesRun(t *testing.T) {
	...
	assert.False(t, resp.Path("errors").Exists(), "unexpected errors: %s", string(resp.Body))
	assert.Equal(t, true, resp.Path("data.upsertBucket.inserted").Value())
...
func TestUpsertBucketUpdatesExistingRun(t *testing.T) {
	...
	assert.Equal(t, false, resp.Path("data.upsertBucket.inserted").Value())
	var run store.Run
